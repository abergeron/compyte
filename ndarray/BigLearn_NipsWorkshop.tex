...
\begin{itemize}
\item Makes a n-dimensional array base object on the GPU that supports \emph{strides} and \emph{broadcasting}
\item Has a Python interface similar to numpy.ndarray (to make it more seamless and appealing to the Python community).
\item Makes the development of efficient specialized operations with this n-dimensional array no harder than with more restricted array objects.
\item Has the base object in C to allow collaboration with other non-Python projects (so that people from C, C++, ruby, R, etc... may use the same low-level base GPU ndarray).
\item Is compatible with CUDA and OpenCL
\end{itemize}

We believe that for an arbitrary-dimensional array is a basic GPU object and that a common implementation is required.
...
\footnote{This can still impact the performance due coalescing constraints(the hardware have different speed for different memory access pattern) , but we don't expect that to be significant most of the time.}
...
They don't support strides nor broadcasting, but they support many data types other then float32.
...
Theano also implemented some optimizations to indexing computation to lower the performance loss of having more dimensions.
...
OpenCL implementation is underway, but currently incomplete.
